...
#define DEVICE_TWIN_UPDATE_STATE_VALUES \
    ...
    typedef void(*IOTHUB_CLIENT_INBOUND_DEVICE_METHOD_CALLBACK)(const char* method_name, const unsigned char* payload, size_t size, uint16_t method_id, void* userContextCallback);
    ...
    */
     ...
     * @return	IOTHUB_CLIENT_OK upon success or an error code upon failure.
     */
     MOCKABLE_FUNCTION(, IOTHUB_CLIENT_RESULT, IoTHubClient_LL_DeviceMethodResponse, IOTHUB_CLIENT_LL_HANDLE, iotHubClientHandle, uint32_t, methodId, const unsigned char*, response, size_t, resp_size, int, status_response);
